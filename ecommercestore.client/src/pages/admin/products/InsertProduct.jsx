// src/pages/admin/products/AddProductBootstrap.jsx
...
export default function AddProductBootstrap() {
    ...
    const [imagesInputKey, setImagesInputKey] = useState(0); // force remount file input
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                setLoading((s) => ({ ...s, categories: false, brands: false }));
            }
        })();
    }, []);
    ...
    const handleImagesAdd = (e) => {
        ...
        const map = new Map(images.map((f) => [fileKey(f), f]));
        for (const f of files) {
            const k = fileKey(f);
            if (!map.has(k)) map.set(k, f);
        }
        const merged = Array.from(map.values()).slice(0, MAX_IMAGES);
        setImages(merged);
    ...
    const validate = () => {
        ...
        if (!avatarFile) er.avatar = "Vui lòng chọn ảnh đại diện";
        setErrors(er);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await productService.createOne(fd);
            alert("Thêm sản phẩm thành công!");
            navigate("/admin/san-pham");
        } catch (err) {
            console.error(err);
            alert(`Lỗi khi thêm sản phẩm: ${err?.message || err}`);
        } finally {
                ...
                <Col lg={12}>
                    <Card>
                        <Card.Header className="d-flex align-items-center justify-content-between">
                            <Card.Title as="h5" className="mb-0">Thêm sản phẩm</Card.Title>
                            ...
                            <Form onSubmit={handleSubmit}>
                                ...
                                <Card className="mt-3">
                                    ...
                                    <Card.Body>
                                        ...
                                        {previewAvatar && (
                                            <div className="mt-3">
                                                <img src={previewAvatar} alt="preview-avatar" className="rounded shadow-sm" style={{ width: 160, height: 160, objectFit: "cover" }} />
                                            </div>
                                        )}
                                    </Card.Body>
                                </Card>
                                ...
                                <Card className="mt-3">
                                    <Card.Header className="d-flex justify-content-between align-items-center">
                                        ...
                                        <Badge bg="secondary">{images.length}/{MAX_IMAGES}</Badge>
                                    </Card.Header>
                                    <Card.Body>
                                        <Form.Group>
                                            <Form.Control
                                                    ...
                                                    <Col key={url}>
                                                        <Card className="h-100">
                                                            <div className="ratio ratio-4x3">
                                                                <img src={url} alt={`preview-${i}`} style={{ objectFit: "cover" }} />
                                                            </div>
                                                            <Card.Body className="d-flex justify-content-between">
                                                                <div className="btn-group btn-group-sm">
                                                                    <Button variant="outline-secondary" disabled={i === 0} onClick={() => moveImage(i, -1)}>←</Button>
                                                                    <Button variant="outline-secondary" disabled={i === images.length - 1} onClick={() => moveImage(i, +1)}>→</Button>
                                                                </div>
                                                                <Button size="sm" variant="outline-danger" onClick={() => removeImageAt(i)}>Xóa</Button>
                                                            </Card.Body>
                                                        </Card>
                                                    </Col>
                                                ))}
                                        ...
                                        <div className="text-muted small mt-2">
                                            Hỗ trợ chọn nhiều lần, tự động gộp & bỏ trùng. Tối đa {MAX_IMAGES} ảnh.
                                        </div>
                                    </Card.Body>
                                </Card>
                                    ...
                                    <Card.Body>
                                        ...
                                        {productSizes.map((sz, idx) => (
                                            <Row key={idx} className="g-2 align-items-end mb-2">
                                                <Col md={6}>
                                                    <Form.Control placeholder={`Tên kích thước ${idx + 1}`} value={sz.sizeName} onChange={changeSize(idx, "sizeName")} />
                                                </Col>
                                                <Col md={4}>
                                                    <Form.Control type="number" placeholder="Số lượng tồn" value={sz.stock} onChange={changeSize(idx, "stock")} />
                                                </Col>
                                            ...
                                            {productColors.map((color, idx) => (
                                                <Col key={idx} md={4} lg={3}>
                                                    <div className="d-flex align-items-center gap-3 border rounded p-2">
                                                        ...
                                                        <div className="small">Mã màu: <strong>{color.toUpperCase()}</strong></div>
                                ...
                                <div className="d-flex gap-2 mt-3">
                                    <Button type="submit" disabled={loading.submitting}>
                                        {loading.submitting ? (<><Spinner size="sm" className="me-2" />Đang lưu...</>) : "Thêm sản phẩm"}
                                    </Button>
